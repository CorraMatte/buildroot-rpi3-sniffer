...
struct __array_of_struct(pcanfd_msg, MAX_MSGS);
#define pcanfd_max_msgs		pcanfd_msgs_MAX_MSGS
						...
						struct pcan_udata *dev_priv)
{
	...
	if (msgfdl.count > MAX_MSGS)
		msgfdl.count = MAX_MSGS;
...
static long __pcan_ioctl(struct file *filep, unsigned int cmd, void __user *up)
{
	...
	case PCANFD_SEND_MSGS:
		err = handle_pcanfd_send_msgs(dev, up, dev_priv);
		break;

	case PCANFD_RECV_MSGS:
		err = handle_pcanfd_recv_msgs(dev, up, dev_priv);
		break;

	case PCANFD_GET_AVAILABLE_CLOCKS:
		err = handle_pcanfd_get_av_clocks(dev, up, dev_priv);
		break;

	case PCANFD_GET_BITTIMING_RANGES:
		err = handle_pcanfd_get_bittiming_ranges(dev, up, dev_priv);
		break;
