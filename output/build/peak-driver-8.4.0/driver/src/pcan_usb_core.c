					...
					struct pcanfd_init *pfdi)
{
	...
	dev->bExtended = !(pfdi->flags & PCANFD_INIT_STD_MSG_ONLY);
                               ...
                               int ctrl_index)
{
	...
	case PCAN_USBCHIP_PRODUCT_ID:
		...
		ucan_soft_init(dev, "usbfd", HW_USB_FD);
		break;
	case PCAN_USBPROFD_PRODUCT_ID:

		/* init structure elements to defaults */
		ucan_soft_init(dev, "usbfd", HW_USB_PRO_FD);
		break;
#ifdef PCAN_USBX6_PRODUCT_ID
	case PCAN_USBX6_PRODUCT_ID:

		/* init structure elements to defaults */
		ucan_soft_init(dev, "usbfd", HW_USB_X6);
		break;
#endif
	case PCAN_USBPRO_PRODUCT_ID:
...
#else
		/* the PCAN-USB adapter already gives this onf at plugin */ 
		dev->device_alt_num = usb_if->ucHardcodedDevNr;
#endif
		break;
	}
